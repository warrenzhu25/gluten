...
if [ -t 1 ]; then
  ...
  docker exec -it $(cat "$storage_filepath") git config --global --add safe.directory /root/incubator-gluten/ep/build-spark/build/spark
  ...
  docker exec -i $(cat "$storage_filepath") git config --global --add safe.directory /root/incubator-gluten/ep/build-spark/build/spark
